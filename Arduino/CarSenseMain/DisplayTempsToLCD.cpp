...
void LcdCharacter(char character) //Prints a charachter to the LCD
{
  LcdWrite(LCD_D, 0x00);
...
void LcdClear(void) //Blanks out the LCD
{
...
void LcdInitialise(void) //Sets up the LCD
{
...
void LcdString(char *characters) // Print a string to the LCD
{
  while (*characters)
  {
...
void LcdWrite(byte dc, byte data) // Send data to the LCD
{
...
void drawLine(void) // Print a line on the LCD
{
  unsigned char  j;  
   for(j=0; j<84; j++) // top
	{
...
float ReadTemp(byte Address) // Grab data from the sensors
{
  int data_low = 0;
  int data_high = 0;
  int pec = 0;
...
void loop(void)  // And the fun begins...
{
  ...
  gotoXY(5,1);
  // Print the header
  LcdString("Left Front");
  //Left Outside
  gotoXY(5,2);
  LcdString("O: ");
  gotoXY(20,2);
  LcdString(dtostrf(ReadTemp(LFOutside), 6, 2, tempBuffer));
  //Left Center
  gotoXY(5,3);
  LcdString("C:  ");
  gotoXY(20,3);
  LcdString(dtostrf(ReadTemp(LFCenter), 6, 2, tempBuffer));
  //Left Inside
  gotoXY(5,4);
  LcdString("I:  ");
  gotoXY(20,4);
  LcdString(dtostrf(ReadTemp(LFInside), 6, 2, tempBuffer));
